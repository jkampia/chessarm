...
class ARM_5DOF:
    ...
    def solveIK(self, target_pose):
        ...
        # 2) Compute effective wrist offset
        wrist_len = l4 + l5
        horz = wrist_len * m.cos(theta4)
        x4 = x - horz * m.cos(theta1)
        y4 = y - horz * m.sin(theta1)
        z4 = z - wrist_len * m.sin(theta4) - l1   # subtract vertical base offset if l1 ≠ 0

        # 3) Distance² from shoulder to wrist
        r2 = x4*x4 + y4*y4 + z4*z4

        # 4) Law‑of‑cosines for elbow (theta2)
        cos_phi = (r2 - l2**2 - l3**2) / (2 * l2 * l3)
        cos_phi = max(-1.0, min(1.0, cos_phi))  # clamp into [-1,1]
        theta2 = -m.acos(cos_phi)

        # 5) Shoulder elevation (theta3)
